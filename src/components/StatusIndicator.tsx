...
import type { FeedbackMode, Language } from '../types';
...
export function StatusIndicator({
  ...
  const isOutputOn = activeMode !== 'none';
  const statusText =
    activeMode === 'daf'
      ? t.modeDaf
      : activeMode === 'faf'
        ...
        <div className="flex items-center gap-4">
          <div
            className={`grid h-16 w-16 place-items-center rounded-xl ${
              isOutputOn ? 'bg-mint text-[#061412]' : 'bg-white/8 text-moss'
            }`}
          >
          ...
          <div>
            ...
            <div className={`mt-1 text-4xl font-black ${isOutputOn ? 'text-mint' : 'text-ink'}`}>{statusText}</div>
          </div>
        </div>
